...
class SpinProjectile(Projectile):

    TTL = 4321

    def __init__(self, pos, projectilePath, explosionPath, colorA, colorB):
        super().__init__(pos)
        ...
        projectileFrames = self.loadFrames(pygame.image.load(projectilePath).convert_alpha(), 8, 8, 6)
        ...
        explosionFrames = self.loadFrames(pygame.image.load(explosionPath).convert_alpha(), 32, 32, 13)
        self.explosionAnimation = Animation(explosionFrames, 100)
        self.explosionAnimation.oneShot = True
        
        self.emitter = PixelEmitter(pos = self.pos.copy(), vel = Vector2(0,0), delay = 500, ttl = 3000, color=colorA)
        self.emitter.velVar = 0.1 
        self.emitter.endColor = colorB
        ...
        self.emitterCountdown.endColor = colorB
    ...
    def explode(self):
        ...
        self.emitterExplode.endColor = self.colorA
